...
class CGdipButtonex : public CButton
{
public:
	void SetOneButton(BOOL bOneButton);
	void SetImagePos(int nStartX, int nStartY);
	void OnDrawWindow(Graphics& Gps);

	CGdipButtonex();
	virtual ~CGdipButtonex();
	...
	BOOL LoadAltImage(UINT id, LPCTSTR pType, HMODULE hInst, int nStartX, int nStartY);
	BOOL LoadStdImage(UINT id, LPCTSTR pType);
	BOOL LoadStdImage(UINT id, LPCTSTR pType, HMODULE hInst, int nStartX, int nStartY);
	BOOL LoadStdImage(LPCTSTR pName);
