...
BOOL CResourcBitmap::Load(CString sFilePathName)
{
	DeleteObject();
...
BOOL CResourcBitmap::Load(UINT ResourceName, CString ResourceType)
{
	//DLL의 인스턴스를 가져와야 한다.  
	HINSTANCE hinstLib;
...
void CResourcBitmap::FreePictureData()
{
	if(m_IPicture != NULL)
	{
		m_IPicture->Release();
...
BOOL CResourcBitmap::LoadPictureData(BYTE *pBuffer, int nSize)
{
	BOOL bResult = FALSE;
...
BOOL CResourcBitmap::CreatePanoramaBitmap()
{
	BOOL bResult = FALSE;
	ILockBytes *Buffer = 0;
	IStorage   *pStorage = 0;
